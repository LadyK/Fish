...
void loop() {
  ...
  */
  ...
  else if ( someoneNear == false && liteAny == true && (birthTime + 40000) > millis() ) { //if any of the timers are still going


    //for (int i = startAddress; i < howManyStructures; i+=3) {
    pixelHue = light(pixelHue, startAddress); // eventually make startAddress  = i
    Serial.println("        Just lighting");
  }
  if ( someoneNear == false && liteAny == true && (birthTime + 20000) < millis() ) {
    fadeDown(pixelHue, startAddress);
    Serial.println("        fading");
  }
  else if ( someoneNear == false && liteAny == true && (birthTime + 40000) < millis() ) {
...
long light(long pH, int address) {
  ...
  for (long j = pH; j < pH + 100; j += 10) { // change hue
...
void turnOff( int address, int wait) {
  ...
  for (int i = address; i < (address + 3); i++) { // For each pixel in strip...
    strip.setPixelColor(i, strip.gamma32(strip.ColorHSV(0, 255, 255)));
  }
